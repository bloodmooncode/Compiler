...
/*在一条ARM数据处理指令中，能用于表示要处理的数据值的位数只能小于32位*/
						  ...
						  std::vector<int> &init_array_index, int dimensions_index)
{
	...
	if (dimensions_index >= dimensions.size())
	{
		...
		if (array.size() > index)
		{ 
			if (ND_ARRAY != array[index]->getType())
			{ 
				//读到了另外一个初始化器
		...
		std::shared_ptr<Node> add = std::dynamic_pointer_cast<Node>(
			std::shared_ptr<OperationNode>(
				new OperationNode(nullptr, ND_OPERATION, OP_ADD, mul,
								  i < indexes_size ? indexes[i] : std::dynamic_pointer_cast<Node>(std::shared_ptr<LiteralNode>(new LiteralNode(nullptr, ND_LITERAL, 0))))));
		mul->setParent(add);
		if (i < indexes_size)
...
// 很多寄存器分配&立即数加载的代码，让编译出来的程序运行更加高效。
